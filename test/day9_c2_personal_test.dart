...
class FakeContentRepository implements ContentRepository {
  ...
  @override
  Future<List<ContentItem>> fetchRecentContent({int limit = 30}) async => [item];
  @override
  Future<List<ContentItem>> searchContent(String query, {int limit = 30, String? contentType}) async => [item];
  @override
  Future<ContentItem?> fetchContentBySlug(String slug) async => item;
}

ContentItem item() => ContentItem(
  id: 'content-1', slug: 'item-1', contentType: 'education_column',
  title: '긴 자료 제목', sourceUrl: 'https://legendstudy.com/item-1', isActive: true,
);

ProviderContainer containerFor({required AuthStatus auth, required FakeBookmarkRepository bookmarks}) =>
...
void main() {
  test('authenticated saved and unsaved state, add/delete, duplicate tap protection', () async {
    ...
    final container = containerFor(auth: const AuthStatus('a'), bookmarks: repo);
    ...
    final controller = container.read(bookmarkStateProvider('content-1').notifier);
    final first = controller.toggle();
    await container.read(authStateProvider.future);
    await Future<void>.delayed(const Duration(milliseconds: 10));
    expect(repo.adds, 1);
    expect(container.read(bookmarkStateProvider('content-1')).phase, BookmarkPhase.mutating);
  ...
  test('guest bookmark does not call repository and auth A/B state is isolated', () async {
    final guestRepo = FakeBookmarkRepository();
    final guest = containerFor(auth: const AuthStatus(null), bookmarks: guestRepo);
    addTearDown(guest.dispose);
    final guestSub = guest.listen(bookmarkStateProvider('content-1'), (_, _) {});
  ...
  testWidgets('resolved detail records recent once and rebuild does not repeat it', (tester) async {
  ...
  testWidgets('recent failure does not hide detail and a fresh entry records again', (tester) async {
    final recent = FakeRecentRepository()..fail = true;
    final value = item();
    Future<void> pumpPage(String slug) => tester.pumpWidget(ProviderScope(
      key: ValueKey(slug),
      overrides: [
        authStateProvider.overrideWith((ref) => Stream.value(const AuthStatus('a'))),
