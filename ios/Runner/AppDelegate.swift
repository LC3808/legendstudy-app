...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  private func configureChannels(binaryMessenger: FlutterBinaryMessenger) {
    ...
    let focus = FlutterMethodChannel(name: "com.legendstudy.app/focus", binaryMessenger: binaryMessenger)
    focus.setMethodCallHandler { call, result in
      do {
        ...
        case "readPreference", "writePreference":
          ...
          let folder = support.appendingPathComponent("StudyFocusLocal", isDirectory: true)
          try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
          var flags = URLResourceValues()
          flags.isExcludedFromBackup = true
          var writableFolder = folder
          try writableFolder.setResourceValues(flags)
    ...
    let channel = FlutterMethodChannel(name: "com.legendstudy.app/study", binaryMessenger: binaryMessenger)
    channel.setMethodCallHandler { call, result in
      do {
        ...
        case "clock":
          ...
          result(["utcMs": Int64(Date().timeIntervalSince1970 * 1000), "elapsedMs": Int64(elapsed), "boot": valid ? "\(boot.tv_sec):\(boot.tv_usec)" : self.processClockIdentity])
        case "read":
