...
-- Ten tables, RLS enabled. FORCE RLS is false in this draft; owners/BYPASSRLS differ from clients.
select c.relname, c.relowner::regrole as owner, c.relrowsecurity, c.relforcerowsecurity, c.relacl
from pg_catalog.pg_class c join pg_catalog.pg_namespace n on n.oid = c.relnamespace
where n.nspname = 'public' and c.relkind = 'r'
  and c.relname in ('source_posts','content_items','exams','subjects','exam_subjects','resources',
                   'profiles','bookmarks','recent_views','ingestion_quarantine')
order by c.relname;
-- Sixteen policies; zero policies on source_posts/ingestion_quarantine, no content writes.
select tablename, policyname, permissive, roles, cmd, qual, with_check
from pg_catalog.pg_policies where schemaname = 'public'
  and tablename in ('source_posts','content_items','exams','subjects','exam_subjects','resources',
                    'profiles','bookmarks','recent_views','ingestion_quarantine')
order by tablename, policyname;
select grantee, table_name, privilege_type, is_grantable
from information_schema.table_privileges where table_schema = 'public'
  and table_name in ('source_posts','content_items','exams','subjects','exam_subjects','resources',
                     'profiles','bookmarks','recent_views','ingestion_quarantine')
...
from pg_catalog.pg_constraint where connamespace = 'public'::regnamespace
  and conrelid in (to_regclass('public.source_posts'),to_regclass('public.content_items'),to_regclass('public.exams'),
...
-- Eight non-internal triggers. Runtime firing still requires separate behavioral tests.
...
-- Ten non-constraint indexes; show constraint indexes separately via constraint_name.
...
where (a.attrelid = to_regclass('public.exams') and a.attname in ('sort_date', 'content_type'))
   or (a.attrelid = to_regclass('public.content_items') and a.attname = 'feed_updated_at');

-- Shared exam PK and same-content composite keys: inspect types/nullability too.
select c.relname as table_name, a.attname, pg_catalog.format_type(a.atttypid, a.atttypmod) as data_type,
