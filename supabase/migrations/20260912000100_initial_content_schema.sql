...
-- pg_trgm absent or installed in extensions; no conflicting application tables.
...
create table public.source_posts (
    ...
    external_post_id text check (external_post_id is null or btrim(external_post_id) <> ''),
...
create table public.exams (
    ...
    source_exam_key text not null check (btrim(source_exam_key) <> ''),
...
create table public.exam_subjects (
    ...
    mapping_status text not null default 'unmapped'
        check (mapping_status in ('unmapped', 'provisional', 'verified')),
    ...
    constraint exam_subjects_mapping_state check (
        (subject_id is null and mapping_status = 'unmapped' and mapping_confidence is null)
        or
        (subject_id is not null and mapping_status in ('provisional', 'verified')
...
create index exams_active_feed on public.exams (year desc nulls last, id desc)
    where is_active;
create index exams_active_filters on public.exams (grade_level, year desc, exam_type, exam_month)
    where is_active;
create index exams_title_search on public.exams using gin (title extensions.gin_trgm_ops)
    where is_active;
create index subjects_parent on public.subjects (parent_id, taxonomy_version);
create index exam_subjects_subject_exam on public.exam_subjects (subject_id, exam_id);
...
revoke all on table public.source_posts, public.exams, public.subjects,
    ...
    public.recent_views from public, anon, authenticated;
grant usage on schema public to anon, authenticated, service_role;
grant select on table public.exams, public.subjects, public.exam_subjects,
    public.resources to anon, authenticated;
...
grant update (display_name, grade_level) on public.profiles to authenticated;
...
grant update (user_id, exam_id, viewed_at) on public.recent_views to authenticated;
...
grant select, insert, update, delete on table public.source_posts, public.exams,
    ...
    public.bookmarks, public.recent_views to service_role;
...
create policy exam_subjects_public_active_select on public.exam_subjects
    for select to anon, authenticated using (
        ...
        and (subject_id is null or exists (
            select 1 from public.subjects s where s.id = subject_id and s.is_active
        ))
    );
...
-- No policy/grant exposes source_posts or permits client content writes.
