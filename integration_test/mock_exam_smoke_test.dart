...
void mark(String stage) => debugPrint('MOCK_FLUTTER PASS $stage');
...
void main() {
  ...
  testWidgets('Mock native storage and optional real JWT runtime', (
    tester,
  ) async {
    ...
    Future<void> press(String name) async {
      ...
      await tester.pumpAndSettle();
      if (name == '시험 시작' && find.text('안내 건너뛰고 시작').evaluate().isNotEmpty) {
    ...
    Future<void> studyForTwoSeconds() async {
      await tab('학습');
      await press('모의고사');
      if (find.text('새 모의고사').evaluate().isNotEmpty) await press('새 모의고사');
      final duration = find.byType(DropdownButtonFormField<String>);
    ...
    try {
      ...
      await current().tick();
      check(
        current().mockPhase == MockPhase.timeUp && current().records.isEmpty,
      );
      check(current().elapsedMs == 60000);
      final frozen = current().draft!.checkpoint;
      await mount(null);
      check(
        current().mockPhase == MockPhase.timeUp &&
            current().draft!.checkpoint == frozen,
      );
      mark(stage);
      await tab('학습');
      ...
      if (credentials != null) {
        ...
        final row = await a
            ...
            .eq('id', savedId)
            .single();
        final local = current().records.single;
        check(
          row['mode'] == 'mock_exam' &&
              row['title'] == local.title &&
              row['planned_duration_seconds'] == 60 &&
              row['duration_seconds'] == local.activeMs ~/ 1000 &&
              jsonEncode(row['active_segments']) ==
                  jsonEncode(local.payload()['active_segments']),
        );
        mark(stage);
      ...
      debugPrint('MOCK_FLUTTER FAIL $stage');
      ...
      transport.close();
      debugPrint('MOCK_FLUTTER ${cleanupOk ? 'PASS' : 'FAIL'} fixture_cleanup');
